...
var vocTemplate = Handlebars.compile(
  fs.readFileSync(__dirname + "/voc.tmpl", "utf-8")
);

// how many images we want to create
const IMAGES_TO_GENERATE = 15;
...
_.each(folders, function (folder, i) {
  ...
  var objs = [];
  objs = _.filter(fs.readdirSync(path.join(FRUITS, folder)), function (
    filename
  ) {
    // only grab png images
...
_.defer(function () {
  var num_completed = 0;
  const progress_threshold = Math.max(
    1,
    Math.round(Math.min(100, IMAGES_TO_GENERATE / 1000))
  );
  async.timesLimit(
    IMAGES_TO_GENERATE,
    CONCURRENCY,
    function (i, cb) {
      createImage(i, function () {
        ...
        if (num_completed % progress_threshold === 0) {
          console.log(
            ((num_completed / IMAGES_TO_GENERATE) * 100).toFixed(1) +
              "% finished."
          );
        }
        cb(null);
...
const createImage = function (filename, cb) {
  ...
  loadImage(path.join(OPEN_IMAGES, BG)).then(function (img) {
    ...
    var objects =
    //   1 + Math.floor(Math.random() * Math.random() * (MAX_OBJECTS - 1));
    2 + Math.floor(Math.random() * (MAX_OBJECTS - 1));
